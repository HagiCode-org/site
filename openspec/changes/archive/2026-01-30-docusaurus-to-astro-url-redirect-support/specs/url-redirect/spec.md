# URL 重定向规格 - Docusaurus 到 Astro 迁移
...
使用构建时生成 HTML 重定向页面的方式,在 `astro build` 后执行自定义脚本:

**脚本文件**: `scripts/generate-redirects.js`
...
**博客文章**:
```
输入: src/content/docs/blog/2026-01-28-slug.md
正则: /^(\d{4})-(\d{2})-(\d{2})-(.+)\.md$/
旧 URL: /blog/2026/01/28/slug
新 URL: /blog/2026-01-28-slug/
```
...
处理:
  ...
  2. 提取分类: quick-start
  3. 提取 slug: create-first-project
旧 URL: /docs/quick-start/create-first-project
新 URL: /quick-start/create-first-project/
```
...
- Node.js >= 18.0 (文件系统操作)
- Astro 5.x (构建流程)
- 无额外 npm 依赖(使用 Node.js 内置模块)

### 环境变量

- `VITE_SITE_BASE`: 站点基础路径,默认 `/`
  - 根路径部署: 不设置或设置为 `/`
  - 子路径部署: 设置为 `/site`
...
- [ ] `/docs/related-software-installation/openspec/setup-openspec` → `/related-software-installation/openspec/setup-openspec/`

#### 部署场景测试
- [ ] 根路径部署 (`npm run build`) 重定向正常
- [ ] 子路径部署 (`npm run build:site`) 重定向正常
...
# 根路径构建
npm run build
ls dist/blog/2026/01/28/*.html
ls dist/docs/quick-start/*.html

# 子路径构建
npm run build:site
# 检查生成的 HTML 中的 URL 包含 /site 前缀
...
- [HTTP 301 Redirect](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/301)
