...
:::tip[版本要求]
本项目要求使用 **OpenSpec 0.23.0** 版本。请按照以下说明安装特定版本，以确保与项目工作流兼容。
:::
...
```bash
sudo npm install -g @fission-ai/openspec@0.23.0
```
...
:::note[Linux 权限问题]
如果遇到 `EACCES` 权限错误，可以：
1. 使用 `sudo`：`sudo npm install -g @fission-ai/openspec@0.23.0`
2. 配置 npm 使用用户目录：
```bash
mkdir -p ~/.npm-global
...
安装完成后，验证 OpenSpec 是否正确安装且版本符合要求：
...
```
openspec/0.23.0 linux-x64 node-v18.17.0
```

:::warning 版本兼容性
请确保安装的是 0.23.0 版本。**不要升级到 1.0 或更高版本**，因为这些版本包含破坏性变更，与当前项目工作流不兼容。
:::
...
cnpm install -g @fission-ai/openspec@0.23.0
```
...
:::warning 版本锁定
本项目要求使用 OpenSpec 0.23.0 版本。**不要升级到其他版本**，除非项目明确要求。
:::
